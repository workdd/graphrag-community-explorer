...
const baseMapping: { [key: string]: string } = {
  ...
  "leiden_communities.parquet": "leiden_community",
...
const useFileHandler = () => {
  ...
  const [leidenCommunities, setLeidenCommunities] = useState<Community[]>([]);
  ...
  const loadFiles = async (files: File[] | string[]) => {
    ...
    for (const file of files) {
      ...
      } else if (schema === "leiden_community") {
        leidenArray.push(data);
    ...
    setLeidenCommunities(leidenArray.flat());
  ...
  return {
    ...
    communities,
    covariates,
    communityReports, leidenCommunities,
    handleFilesRead,
    loadDefaultFiles,
  };
};
