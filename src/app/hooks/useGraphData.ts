...
import { buildMembership } from "../utils/community-utils";
...
const useGraphData = (
  ...
  useEffect(() => {
    ...
    // Membership is attached to every entity so community colouring and focus
    // work even when the community nodes themselves are not drawn.
    const membership = buildMembership(communities);

    // Sort entities by relationship count and limit to maxEntities if specified
    let filteredEntities = hideUnclustered
      ? entities.filter((entity) => membership.has(entity.id))
      : entities;
    if (maxEntities > 0 && filteredEntities.length > maxEntities) {
    ...
    const nodes: CustomNode[] = filteredEntities.map((entity) => {
      ...
      return {
        ...
        community_ids: (member?.all ?? []).map((c) => c.id.toString()),
        community_titles: (member?.all ?? []).map((c) => c.title),
        community_number: member?.primary.community,
        neighbors: [],
        links: [],
      };
    });
    ...
    if (includeCommunities) {
      const communityNodes = communities.map((community) => {                
        ...
        return {
          ...
          size: community.size ?? community.entity_ids?.length ?? 0,
          community_number: community.community,
          parent: community.parent,
          full_content: report?.full_content || "",
          level: community.level ?? report?.level ?? -1,
          rank: report?.rank ?? -1,
          title: report?.title || community.title || "",
      ...
      const communityEntityLinks = communities
        .flatMap((community) => {
          // Explicit membership is authoritative, including an empty member list.
          // Inferring from internal edges loses isolated members and can add outsiders.
          if (Array.isArray(community.entity_ids)) {
            return community.entity_ids.flatMap((entityId) => {
      ...
      communities.forEach((community) => {
        if (community.parent == null || community.parent < 0) return;
        const parent = communities.find((c) => c.community === Number(community.parent) || c.id.toString() === String(community.parent));
        if (parent) links.push({
          source: community.id.toString(), target: parent.id.toString(),
          type: "CHILD_OF", id: `parent:${community.id}:${parent.id}`,
        });
      });
