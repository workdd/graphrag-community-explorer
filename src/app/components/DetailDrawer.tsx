...
interface DetailDrawerProps {
  ...
  onFocusCommunity?: (node: CustomNode) => void;
}
    ...
    >
      <Box sx={{ width: "100%", padding: 3 }}>
        ...
        >
          ...
          {selectedNode?.type === "COMMUNITY" && onFocusCommunity && (
            <Button
              variant="outlined"
              size="small"
              sx={{ marginLeft: 2 }}
              onClick={() => onFocusCommunity(selectedNode)}
            >
              Focus Community
            </Button>
          )}
            ...
            <CardContent>
              ...
              {selectedNode.type === "COMMUNITY" &&
                selectedNode.size !== undefined && (
                  <Typography>Members: {selectedNode.size}</Typography>
                )}
              {selectedNode.community_titles &&
                selectedNode.community_titles.length > 0 && (
                  <Typography>
                    Communities: {selectedNode.community_titles.join(", ")}
                  </Typography>
                )}
              {selectedNode.title && (
                <Typography>
