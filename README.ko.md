...
| 이럴 때 | 이 화면 |
| --- | --- |
| 이 색인 위에 무언가를 만들어도 되는지 판단할 때 | **개요** 와 **품질**. 레벨별 커버리지와 모듈성, 크기 분포, 어디에도 속하지 않은 엔티티, 무결성 문제를 하나씩 이름 붙여 보여줍니다. 엔티티의 4분의 1이 어느 커뮤니티에도 없다면 Global 검색은 그것들을 영영 보지 못합니다. 그 사실은 나중보다 지금 아는 편이 낫습니다 |
...
| 남이 만든 색인을 넘겨받았을 때 | **스키마** 부터. 엔티티 유형, 그 사이에 실제로 존재하는 관계, 그리고 키·참조 열이 표시된 Parquet 테이블. 이 형태를 선언하는 곳은 없고 행에서 세어 만듭니다 |
